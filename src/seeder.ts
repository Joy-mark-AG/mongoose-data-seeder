...
class MongooseDataSeeder {
  private chunks = {};
  private sandbox = vm.createContext();

  // Pass the connection from above to make sure we're using the correct models.
  constructor(public connection: Mongoose, public dropCollection = false) {}
  ...
  asyncSeries(tasks) {
    if (!(tasks instanceof Array) || tasks.length === 0) {
      return Promise.reject(
        new Error(`Please pass tasks array into this function.`)
      );
    }
  ...
  requireDeps(data) {
    try {
      const deps: any = data._dependencies;
      ...
      Object.entries(deps).forEach(([key, value]) => {
        ...
        this.sandbox[key] = module.parent.require(value as string);
      });
    ...
    if (value === undefined) {
      ...
      try {
        ...
        return vm.runInContext(
          value.substr(1).replace(/this\./g, "_this."),
          context
        );
      } catch (err) {
        return value;
  ...
  findReference(ref) {
    ...
    if (typeof result === "object" && !(result instanceof Array)) {
      ...
      if (!result._id) {
        ...
        throw new TypeError(
          `Could not read property "_id" of ${JSON.stringify(result)}`
        );
      }
    ...
    const tasks = Object.keys(data).map((key) => () => {
      ...
      try {
        if (!value._model) {
          ...
          throw new Error(
            "Please provide a _model property that describes which database model should be used."
          );
        }
            ...
            if (this.dropCollection) {
              ...
              return this.connection.connection.db
                ...
                .then((collections) => {
                  ...
                  return isExitst
                    ? this.connection.connection.dropCollection(
          ...
          .then(() => {
            const innerTasks = Object.keys(value).map((innerKey) => () => {
              ...
              if (Model.schema.paths["_id"]) {
                ...
                if (!items["_id"]) {
                  items["_id"] = this.connection.Types.ObjectId();
                }
              }

              // Create the model
              return Model.create(items).then((result) => {
